...
# Goal: Replace the iterative version of C(n,k) with a recursive function
...
#======== C(N,K) FUNCTION
# Registers used:
#   $a0 function parameter n 20(sp)
#   $a1 function parameter k 24(sp)
#   $a2 local C( n-1, k ) 28(sp)
...
cnk:
  ...
  sw $a0, 20($sp)		# save n to the stack
  sw $a1, 24($sp)  		# save k to the stack

  sub $a0, $a0, 1		# decrement n
  jal cnk			# call the recursive function
  sw $v0, 28($sp)		# save result from cnk(n-1,k)

  # reload the values of n and k from the stack
  lw $a0, 20($sp)
  lw $a1, 24($sp)

  sub $a0, $a0, 1		# decrement n
  sub $a1, $a1, 1		# decrement k

  jal cnk			# call recursive function for n-1 and k-1

  lw $a2, 28($sp)		# grab the value of cnk(n-1,k)
  add $v0, $v0, $a2		# add the results from the two recursive calls
